...
func (h *TCPSession) Start(conn *net.TCPConn) {
    ...
    go func() {
        ...
        // just to make sure
        h.conn.Close()
        // make sure all events generated by outstanding Timeouts are sunk
        go h.drain_events()
        // stop all Timeouts, making sure they won't try to use h.Events
        h.release_timeouts()
        // disengage user and stop drain_events()
        close(h.Events)
...
func (h *TCPSession) release_timeouts() {
    for {
        ...
        // when this call returns, it is guaranteed that Timeout won't post on h.Events
        to.Free()
